...
var _header = UICreate_Header(_xx, NAV_TOP_Y, "game paused" );
with _header
{
	SetDelay(4);	
}
...
_yy += 224;
var _inst = UICreate_SettingsButton( "contents quit", _xx, _yy )
with ( _inst )
{
	SetDelay(16)
	SetWidthFixed( 420, 0 );
	
	SetState(SETTINGSBUTTON.TOGGLE, Button_QuitLevel);
	//toggleState = VaultRead(SAVEDATA_INPUTS_HAPTICS, true);
	
	SetBlend(c_white, C_BROXCORP_RED);
	Apply();
}