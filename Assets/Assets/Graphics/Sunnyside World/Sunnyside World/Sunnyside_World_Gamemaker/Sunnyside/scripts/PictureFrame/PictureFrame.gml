function PictureFrame() constructor
{
    ...
    static SetGuiParams = function(_guiTargetWidth, _guiTargetHeight)
    {
        ...
        {
            TraceError("May only specify either a GUI width target or a GUI height target, not both");
        }
    ...
    static Apply = function()
    {
        ...
        if ((surface_get_width(application_surface) != GetViewWidth()) || (surface_get_width(application_surface) != GetViewHeight()))
        {
    ...
    static DrawApplicationSurface = function(_filter)
    {
        Recalculate();
        with(__output)
        {
    ...
    static Recalculate = function()
    {
        ...
        if (DEV_CONFIG) Trace(GetDebugString());
