...
void main()
{
    ...
    if ((len < u_fMaxRadius) && (len > u_fMinRadius))
    {
        ...
        float midRadius = 0.5*(u_fMaxRadius + u_fMinRadius);
        float alpha = 1.0 - abs(2.0*(midRadius - len) / thickness);
        alpha = pow(alpha, u_fExponent);
        
        gl_FragColor = vec4(delta, 1.0, v_vColour.a*alpha);
    }
    else
    {
